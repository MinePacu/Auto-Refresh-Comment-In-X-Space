...
> ⚠️ **알림**: 개발 관련 문서, 설정 옵션, 주의 사항 및 제한 사항은 곧 별도의 "위키(Wiki) 페이지"로 이동될 예정입니다. 최신 정보와 상세 안내는 추후 위치 페이지에서 확인해 주세요.
...
- **클릭 간 대기시간**: 5ms~10000ms 범위에서 미세 조정 (기본값: 700ms)
...
## 📢 안내: 개발, 설정 옵션, 주의사항/제한사항 안내 예정

> ⚠️ **개발 관련 정보, 설정 옵션, 주의사항 및 제한사항은 곧 별도의 "위키(Wiki) 페이지"로 이동될 예정입니다.**
> 
> - 앞으로는 해당 페이지에서 최신 개발 문서, 고급 설정, 주의사항, 제한사항 등을 확인하실 수 있습니다.
> - README에는 핵심 사용법과 요약 정보만 제공될 예정입니다.
> - 자세한 내용은 추후 안내되는 위치 페이지 링크를 참고해 주세요.
...
| **클릭 간 대기시간** | 5-10000ms | 700ms | UI 요소 클릭 사이 지연시간 | 500-1000ms |
...
- **최솟값**: 5ms (5ms 미만 입력 시 자동으로 5ms로 조정)
...
> ⚠️ **참고**: 개발, 설정 옵션, 주의 사항 및 제한 사항에 대한 자세한 내용은 곧 별도의 위키(Wiki) 페이지로 이전될 예정입니다. 해당 페이지에서 최신 정보를 확인해 주세요.
...
4. **클릭 간 대기시간 조정**:
   - 5ms 미만 설정 시 자동으로 5ms로 조정되는 것은 정상 동작
   - 빠른 클릭이 필요한 경우에도 최소 5ms는 유지됨
...
6. **디버그 및 문제 진단**:
   - 개발자 옵션에서 콘솔 로그를 활성화
   - 브라우저 개발자 도구(F12)에서 상세한 실행 로그 확인
