...
### 🎯 인텔리전트 감지
- **페이지 감지**: X 스페이스 상세 페이지 자동 인식 (URL 패턴 및 DOM 요소 기반)
- **상태 인식**: 스페이스 청취 상태 실시간 감지 및 다중 조건 검증
- **스페이스 참여 감지**: "참여했습니다", "스페이스 청취 중", "Join this Space" 등 다양한 언어 지원
- **생방송 대기 상태 감지**: "생방송으로 듣기"/"Listen live" 버튼 감지로 대기 상태와 청취 상태 구분
- **조건부 실행**: 모든 조건 충족 시에만 자동 활성화
- **SPA 지원**: 페이지 이동 시 즉시 상태 반영 및 자동 제어

### 🚀 사용자 경험
- **마스터 제어**: ON/OFF 토글로 모든 기능을 완전히 제어 (OFF 시 모든 활동 중단)
- **무음 모드**: 마스터 OFF 시 완전한 조용한 상태 유지 (로그 출력 최소화)
- **실시간 피드백**: 상태별 명확한 메시지 표시 및 로딩 스피너
- **스마트 로깅**: 마스터 ON 상태에서만 상세 디버깅 정보 출력
...
- **상태: 비활성화됨**: 마스터 OFF 상태 (모든 기능 정지, 조건 확인 중단)
- **트위터 접속 대기 중**: X 사이트가 아닌 페이지에 있음
- **트윗 상세 페이지 대기 중**: X에는 접속했으나 스페이스 상세 페이지가 아님
- **스페이스 청취 대기 중**: 스페이스 페이지에는 있으나 청취 상태가 아님 (생방송 대기 상태 포함)

#### 스페이스 청취 상태 감지 기준
- ✅ **청취 중으로 판단**: "참여했습니다", "스페이스 청취 중", "Join this Space", "Listening to this Space" 표시
- ❌ **대기 중으로 판단**: "생방송으로 듣기", "Listen live" 버튼이 보이는 경우
- 🔍 **추가 감지**: 스페이스 플레이어, 오디오 컨트롤, SVG 배지 등을 통한 다중 검증
...
- **스페이스 감지가 안 되는 경우**: 
  ...
  - "생방송으로 듣기" 버튼이 있는지 확인 (이 경우 대기 상태로 인식됨)
  - 실제로 스페이스를 청취하고 있는지 확인
- **반복 실행이 멈추는 경우**:
  - 마스터 OFF → ON 토글로 재시작
  - 클릭 간 대기시간을 늘려보기 (1000ms 이상)
- **콘솔 로그가 보이지 않는 경우**:
  - 마스터가 ON 상태인지 확인 (OFF 상태에서는 로그 출력 최소화)
...
- **감지 기능**: 
  - URL 변경 감지 (SPA 지원, history API 오버라이드)
  - 스페이스 청취 상태 감지 (다중 조건 검증)
  - 페이지 조건 확인 (트윗 상세 페이지 패턴 인식)
  - "생방송으로 듣기" vs "청취 중" 상태 구분
- **자동화 로직**:
  - 조건 충족 시 자동 시작 (3초마다 조건 확인)
  - 반복 정렬 작업 수행 (사용자 설정 주기)
  - 마스터 OFF 시 완전 정지 (모든 인터벌 및 감지 중단)
  - XPath 기반 DOM 요소 정확한 선택
- **오류 처리**: 
  - 마스터 상태 중간 체크로 안전한 중단
  - DOM 요소 대기 및 timeout 처리
  - 네트워크 오류 시 재시도 로직
...
- **마스터 제어**: OFF 상태에서는 모든 기능 완전 중단 (조건 확인, 로그 출력 최소화)
...
4. **스페이스 감지 문제**:
   - 브라우저 개발자 도구(F12) → Console 탭에서 로그 확인
   - "생방송으로 듣기" 버튼이 있는지 확인 (대기 상태로 인식됨)
   - 실제로 스페이스를 청취하고 있는지 확인
...
- 마스터 버튼 상태 (ON/OFF)
- 현재 X 페이지 URL
- 스페이스 청취 상태 ("생방송으로 듣기" 버튼 유무 등)

#### 디버깅 정보 수집 방법
1. **개발자 도구 열기**: F12 키 또는 우클릭 → "검사"
2. **Console 탭 선택**: 로그 메시지 확인
3. **마스터 ON**: 상세 로그가 마스터 ON 상태에서만 출력됨
4. **중요 로그**: "스페이스 청취 감지", "페이지 상태 확인", "메인 기능 로직 시작" 메시지 확인
